...
/**
 * Created by TomLeisen on 2022/7/24 1:33 上午
 * Email: xy162162a@163.com
 * Description:圆角、圆形、带边框 各种图片
 */
...
//        // 绘制边框
        drawBorders(canvas);
        if (onDrawListener != null) onDrawListener.doOnDraw();
    }
    ...
    public void setInnerBorderWidth(int innerBorderWidth) {
        this.innerBorderWidth = dip2px(context, innerBorderWidth);
        clearInnerBorderWidth();
        invalidate();
    }
    ...
    public void setCornerRadius(int cornerRadiusDP) {
        this.cornerRadius = dip2px(context, cornerRadiusDP);
        invalidateOutline();
        calculateRadiiAndRectF(false);
    }

    public void setCornerTopLeftRadius(int cornerTopLeftRadius) {
        this.cornerTopLeftRadius = dip2px(context, cornerTopLeftRadius);
        calculateRadiiAndRectF(true);
    }

    public void setCornerTopRightRadius(int cornerTopRightRadius) {
        this.cornerTopRightRadius = dip2px(context, cornerTopRightRadius);
        calculateRadiiAndRectF(true);
    }

    public void setCornerBottomLeftRadius(int cornerBottomLeftRadius) {
        this.cornerBottomLeftRadius = dip2px(context, cornerBottomLeftRadius);
        calculateRadiiAndRectF(true);
    }

    public void setCornerBottomRightRadius(int cornerBottomRightRadius) {
        this.cornerBottomRightRadius = dip2px(context, cornerBottomRightRadius);
        calculateRadiiAndRectF(true);
    }
    ...
    public static int dip2px(Context context, float dpValue) {
        if (context == null || dpValue == 0) return 0;
